...
void setup() 
{
 ...
 // SPI setup
  ...
  digitalWrite(DHVpin, LOW); // off MAX1771 Driver  Hight Voltage(DHV) 110-220V
  //setRTCDateTime(RTC_hours,RTC_minutes,RTC_seconds,RTC_day,RTC_month,RTC_year,1); //ÑxÑpÑÅÑyÑÉÑçÑrÑpÑuÑ} ÑÑÑÄÑ|ÑéÑ{ÑÄ ÑâÑÑÑÄ ÑÉÑâÑyÑÑÑpÑ~Ñ~ÑÄÑu ÑrÑÇÑuÑ}Ñë Ñr RTC ÑâÑÑÑÄÑqÑç ÑxÑpÑÅÑÖÑÉÑÑÑyÑÑÑé Ñ~ÑÄÑrÑÖÑê Ñ}ÑyÑ{ÑÇÑÄÑÉÑáÑuÑ}ÑÖ
  digitalWrite(DHVpin, HIGH); // on MAX1771 Driver  Hight Voltage(DHV) 110-220V
  //p=song;
}
...
//int minuteL=0; //Ñ}Ñ|ÑpÑtÑäÑpÑë ÑàÑyÑÜÑÇÑp Ñ}ÑyÑ~ÑÖÑÑ
...
void dicrementValue()
{
	  ...
	  if (menuPosition==Alarm01) 
		{
		 if (value[menuPosition]==1) /*digitalWrite(pinUpperDots, HIGH);*/ dotPattern=B10000000;//turn on upper dots ÑrÑ{Ñ|ÑêÑâÑpÑuÑ} ÑrÑuÑÇÑáÑ~ÑyÑu ÑÑÑÄÑâÑ{Ñy
